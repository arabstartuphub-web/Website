...
function extractImageUrl(item: Rss2JsonItem): string | null {
  // 1. rss2json's own thumbnail field (most common)
  if (item.thumbnail && item.thumbnail.startsWith("http")) return item.thumbnail;

  // 2. <enclosure> tag (used by many news feeds)
  ...
  // 3. Scrape first <img src="..."> out of description/content HTML
  const html = item.content || item.description || "";
  const imgMatch = html.match(/<img[^>]+src=["']([^"']+)["']/i);
  if (imgMatch?.[1] && imgMatch[1].startsWith("http")) return imgMatch[1];

  // 4. og:image style meta in content
...
export async function fetchAndStoreFeed(feed: Feed): Promise<number> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      // ── Extract real image from the feed item ──────────────────────────────
